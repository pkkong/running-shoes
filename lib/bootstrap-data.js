...
const DEFAULT_TABLE_PREFIX = "runfit_";
...
async function loadSupabaseBootstrap() {
  ...
  const [shoeRows, periodRows, entryRows, priceRows] = await Promise.all([
    fetchSupabase(supabaseUrl, supabaseKey, `/rest/v1/${tableName(tablePrefix, "shoes")}?select=data&order=sort_order.asc`),
    fetchSupabase(supabaseUrl, supabaseKey, `/rest/v1/${tableName(tablePrefix, "lineup_periods")}?select=data&order=sort_order.asc`),
    fetchSupabase(supabaseUrl, supabaseKey, `/rest/v1/${tableName(tablePrefix, "lineup_items")}?select=data&order=sort_order.asc`),
    fetchSupabase(supabaseUrl, supabaseKey, `/rest/v1/${tableName(tablePrefix, "price_query_config")}?select=data&id=eq.default&limit=1`),
  ]);
...
function normalizeTablePrefix(value) {
  const prefix = value === undefined ? DEFAULT_TABLE_PREFIX : String(value);
  if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(`${prefix}table`)) {
    throw new Error("Invalid Supabase table prefix");
  }
  return prefix;
}
